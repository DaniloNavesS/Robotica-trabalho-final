...
# (use o nome exato dos dummies criado na cena)
...
# (Opcional: pegar handle da base do UR3 para usar como frame de referência)
res, baseHandle = sim.simxGetObjectHandle(clientID, 'UR3', sim.simx_opmode_blocking)  # ou o nome do base frame
if res != sim.simx_return_ok:
    baseHandle = -1  # usar sistema de coordenadas mundial

# Definir coordenadas dos vértices do quadrado (no plano XY, próximo ao chão, z=0.05 m)
square_points = [
    [0.3,  0.0, 0.20],   # ponto 1
    [0.3,  0.2, 0.20],   # ponto 2
    [0.5,  0.2, 0.20],   # ponto 3
    [0.5,  0.0, 0.20],   # ponto 4
    [0.3,  0.0, 0.20]    # volta ao ponto inicial
]

# Mover o dummy de meta para cada ponto do quadrado
for pos in square_points:
    # pos é [x, y, z] em metros; -1 indica sistema de coordenadas global,
    # ou use baseHandle para referenciar ao robô (visto acima)
    sim.simxSetObjectPosition(clientID, targetHandle, -1, pos, sim.simx_opmode_oneshot)
